...
type MetricKey = 'newMembers' | 'revenue' | 'arpu' | 'arppu'
...
const METRICS: { key: MetricKey; label: string; color: string; axis: 'left' | 'right'; type: 'bar' | 'line' }[] = [
  ...
  { key: 'newMembers', label: '신규 유저', color: '#3b82f6', axis: 'right', type: 'bar'  },
]
...
export default function NewUsersTrendChart({ data }: { data: DailyPoint[] }) {
  const [active, setActive] = useState<Set<MetricKey>>(
    new Set(['newMembers', 'revenue', 'arpu', 'arppu'])
  )

...
export default function AllUsersRatioChart({ data }: { data: DailyPoint[] }) {
  ...
  const ratioData = useMemo(() =>
    source.map(d => ({
      ...
      ratio: d.dau > 0 ? Math.round((d.payingUsers / d.dau) * 1000) / 10 : 0,
    })),
  [source])
  ...
  return (
    <div className="bg-bg-secondary border border-line rounded-lg p-5 h-full flex flex-col">
      <h3 className="text-base font-bold mb-4">
        결제 전환율 (PUR)
        <span className="ml-1.5 text-xs font-normal text-text-secondary">(결제 유저 / DAU)</span>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={ratioData} margin={{ top: 4, right: 8, left: 0, bottom: 4 }}>
            ...
            <Tooltip
              ...
              formatter={(v) => [`${v}%`, 'PUR']}
            />
            <Line type="monotone" dataKey="ratio" stroke="#f59e0b" strokeWidth={2} dot={false} activeDot={{ r: 4 }} />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </div>

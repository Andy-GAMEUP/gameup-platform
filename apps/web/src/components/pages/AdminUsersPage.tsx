    ...
    <AdminLayout>
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-text-primary text-xl font-bold">회원 관리</h2>
          <span className="text-text-secondary text-sm">{loading ? '로딩 중...' : `총 ${total}명`}</span>
        </div>
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <input
            ...
            className="bg-bg-tertiary border border-line text-text-primary rounded px-3 py-1.5 text-sm placeholder-text-muted flex-1 max-w-xs"
          />
        </div>

        <div className="bg-bg-secondary rounded-lg border border-line overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={7} className="text-center py-8 text-text-muted">회원이 없습니다</td></tr>
              ) : users.map((user) => {
                const rl = ROLE_LABELS[user.role] || { label: user.role, color: 'text-text-secondary', bg: 'bg-bg-tertiary' }
                return (
                  <tr key={user._id} className="border-b border-line/50 hover:bg-bg-tertiary/30">
                    ...
                    <td className="px-4 py-3">
                      {user.isActive ? (
                        <span className="text-accent text-xs">활성</span>
                      ) : (
                        <span className="text-accent-text text-xs">정지됨</span>
                      )}
                    </td>
      ...
      {modalUser && modalAction === 'role' && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" onClick={() => setModalUser(null)}>
          <div className="bg-bg-secondary border border-line rounded-xl p-6 w-full max-w-sm mx-4" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-text-primary font-bold text-lg mb-4">역할 변경</h3>
            <p className="text-text-secondary text-sm mb-4"><span className="text-text-primary">{modalUser.username}</span>의 역할을 변경합니다</p>
            <div className="space-y-2 mb-4">
              {['developer', 'player', 'admin'].map((r) => (
                <label key={r} className={`flex items-center gap-3 p-3 rounded border cursor-pointer transition-colors ${newRole === r ? 'border-accent-muted bg-red-600/10' : 'border-line hover:border-line'}`}>
                  <input type="radio" value={r} checked={newRole === r} onChange={() => setNewRole(r)} className="accent-red-500" />
                  <span className="text-text-secondary capitalize">{{ developer: '개발자', player: '플레이어', admin: '관리자' }[r]}</span>
                </label>
              ))}
            </div>
      ...
      {modalUser && modalAction === 'ban' && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50" onClick={() => setModalUser(null)}>
          <div className="bg-bg-secondary border border-line rounded-xl p-6 w-full max-w-sm mx-4" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-text-primary font-bold text-lg mb-4">사용자 정지</h3>
            <p className="text-text-secondary text-sm mb-4"><span className="text-text-primary">{modalUser.username}</span>을 정지하시겠습니까?</p>
            <div className="space-y-3 mb-4">
              <div>
                <label className="block text-text-secondary text-xs mb-1">정지 사유</label>
                <textarea value={banReason} onChange={(e) => setBanReason(e.target.value)} rows={2} className="w-full bg-bg-tertiary border border-line text-text-primary rounded px-3 py-2 text-sm" placeholder="정지 사유..." />
              </div>
              <div>
                <label className="block text-text-secondary text-xs mb-1">정지 종료일 (선택, 미입력시 영구 정지)</label>
                <input type="date" value={bannedUntil} onChange={(e) => setBannedUntil(e.target.value)} className="w-full bg-bg-tertiary border border-line text-text-primary rounded px-3 py-2 text-sm" />
              </div>
            </div>
            <div className="flex gap-3">

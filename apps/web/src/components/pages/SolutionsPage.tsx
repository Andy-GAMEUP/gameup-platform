...
function SolutionCard({ solution, onClick }: { solution: Solution; onClick: () => void }) {
  ...
  return (
    <button
      onClick={onClick}
      className="bg-bg-secondary border border-line hover:border-line rounded-2xl overflow-hidden text-left transition-all hover:scale-[1.02] hover:shadow-xl hover:shadow-black/40 group"
    >
      ...
      <div className="p-4">
        ...
        <h3 className="text-text-primary font-bold text-base mb-1">{solution.name}</h3>
        <p className="text-text-secondary text-sm line-clamp-2">{solution.description}</p>
      </div>
    </button>
  )
}
  ...
  return (
    <div className="min-h-screen bg-bg-primary">
      ...
      <div className="max-w-7xl mx-auto px-6 py-16">
        {loading ? (
          <div className="text-center py-20 text-text-secondary">불러오는 중...</div>
        ) : (
          <>
            {recommended.length > 0 && (
              <section className="mb-16">
                <div className="flex items-center gap-3 mb-6">
                  ...
                  <h2 className="text-text-primary text-2xl font-bold">추천 솔루션</h2>
                </div>
            ...
            <section className="mb-16">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-text-primary text-2xl font-bold">전체 솔루션</h2>
                <span className="text-text-muted text-sm">({all.length})</span>
              </div>
              {all.length === 0 ? (
                <div className="text-center py-20 text-text-muted">등록된 솔루션이 없습니다.</div>
              ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                  {all.map(s => (
                    <SolutionCard key={s._id} solution={s} onClick={() => setSelectedId(s._id)} />
        ...
        <section className="mb-16">
          <h2 className="text-text-primary text-2xl font-bold text-center mb-10">Why GameUp 솔루션?</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="bg-bg-secondary border border-line rounded-2xl p-6 text-center">
              <div className="w-12 h-12 bg-blue-500/20 rounded-xl flex items-center justify-center mx-auto mb-4">
                <Shield className="w-6 h-6 text-blue-400" />
              </div>
              <h3 className="text-text-primary font-bold text-lg mb-2">검증된 파트너</h3>
              <p className="text-text-secondary text-sm">엄격한 검증을 통과한 신뢰할 수 있는 솔루션 파트너사와 함께합니다.</p>
            </div>
            <div className="bg-bg-secondary border border-line rounded-2xl p-6 text-center">
              <div className="w-12 h-12 bg-accent-light rounded-xl flex items-center justify-center mx-auto mb-4">
                <Zap className="w-6 h-6 text-accent" />
              </div>
              <h3 className="text-text-primary font-bold text-lg mb-2">빠른 연동</h3>
              <p className="text-text-secondary text-sm">간단한 신청 절차로 빠르게 솔루션을 도입하고 개발을 가속화하세요.</p>
            </div>
            <div className="bg-bg-secondary border border-line rounded-2xl p-6 text-center">
              <div className="w-12 h-12 bg-purple-500/20 rounded-xl flex items-center justify-center mx-auto mb-4">
                <HeadphonesIcon className="w-6 h-6 text-purple-400" />
              </div>
              <h3 className="text-text-primary font-bold text-lg mb-2">전담 지원</h3>
              <p className="text-text-secondary text-sm">솔루션 도입부터 운영까지 GameUp 담당자가 함께 지원합니다.</p>
            </div>
          </div>
        </section>

        <section className="rounded-2xl bg-gradient-to-r from-red-900/40 to-rose-900/40 border border-red-500/20 p-10 text-center">
          <h2 className="text-text-primary text-2xl font-bold mb-3">원하는 솔루션이 없으신가요?</h2>
          <p className="text-text-secondary mb-6">GameUp 파트너팀에 문의하시면 맞춤 솔루션을 안내해드립니다.</p>
          <a
            href="mailto:partner@gameup.co.kr"
            className="inline-flex items-center gap-2 bg-red-600 hover:bg-red-500 text-text-primary font-semibold px-6 py-3 rounded-xl transition-colors"
          >

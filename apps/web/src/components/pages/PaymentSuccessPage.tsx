      ...
      <div className="bg-bg-secondary border border-line rounded-xl w-full max-w-md p-8 text-center">
        ...
        {status === 'success' && (
          <div className="flex flex-col items-center gap-4">
            ...
            {paymentInfo && (
              <div className="w-full p-4 bg-bg-tertiary/50 rounded-lg border border-line text-left space-y-2 mt-2">
                {paymentInfo.orderName && (
                  <div className="flex justify-between text-sm">
                    <span className="text-text-secondary">상품</span>
                    <span className="text-text-primary font-medium">{paymentInfo.orderName}</span>
                  </div>
                )}
                {paymentInfo.amount !== undefined && (
                  <div className="flex justify-between text-sm">
                    <span className="text-text-secondary">결제 금액</span>
                    <span className="text-accent font-bold">₩{paymentInfo.amount.toLocaleString()}</span>
                  </div>
                )}
                {paymentInfo.method && (
                  <div className="flex justify-between text-sm">
                    <span className="text-text-secondary">결제 수단</span>
                    <span className="text-text-primary">{paymentInfo.method}</span>
                  </div>
                )}
                {paymentInfo.approvedAt && (
                  <div className="flex justify-between text-sm">
                    <span className="text-text-secondary">결제 일시</span>

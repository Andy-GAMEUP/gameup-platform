  ...
  if (!partner) return (
    <div className="min-h-screen bg-bg-primary">
      ...
      <div className="text-center py-24">
        <p className="text-text-secondary">파트너 채널을 찾을 수 없습니다</p>
  ...
  return (
    <div className="min-h-screen bg-bg-primary">
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        <Link href="/partner" className="flex items-center gap-1.5 text-text-secondary hover:text-text-primary text-sm mb-6 transition-colors">
          <ArrowLeft className="w-4 h-4" /> 파트너 목록
        </Link>

        <div className="bg-bg-secondary border border-line rounded-xl p-6 mb-6">
          <div className="flex items-start gap-5">
            {partner.profileImage ? (
              <Image src={partner.profileImage} alt={username} width={80} height={80} className="w-20 h-20 rounded-full object-cover border border-line flex-shrink-0" unoptimized />
            ) : (
              <div className={`w-20 h-20 rounded-full flex items-center justify-center text-text-primary font-bold text-2xl flex-shrink-0 ${avatarBg}`}>
                {username[0].toUpperCase()}
              </div>
            )}
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-3 flex-wrap">
                <h1 className="text-text-primary text-xl font-bold">{username}</h1>
                <span className="bg-cyan-600/20 text-cyan-400 text-xs px-2 py-0.5 rounded border border-cyan-500/30">파트너</span>
              </div>
              {partner.slogan && (
                <p className="text-text-secondary text-sm mt-1">{partner.slogan}</p>
              )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          <aside className="lg:col-span-1 space-y-3">
            <div className="bg-bg-secondary border border-line rounded-xl p-4">
              <p className="text-text-secondary text-xs font-semibold mb-3 uppercase tracking-wide">정렬</p>
          ...
          <main className="lg:col-span-3 space-y-3">
            {postsLoading ? (
              <div className="flex justify-center py-12"><Loader2 className="w-6 h-6 animate-spin text-text-secondary" /></div>
            ) : posts.length === 0 ? (
              <div className="bg-bg-secondary border border-line rounded-xl p-12 text-center">
                ...
                {isMyChannel && (
                  <Link href="/partner/write"
                    className="mt-4 inline-block bg-cyan-600 hover:bg-cyan-700 text-text-primary px-4 py-2 rounded-lg text-sm">
                    첫 글 작성하기
                  </Link>
                )}
              </div>
...
function PostCard({ post, partnerId }: { post: PartnerPostItem; partnerId: string }) {
  ...
  return (
    <Link href={`/partner/${partnerId}/${post._id}`}
      className="block bg-bg-secondary border border-line hover:border-cyan-500/30 rounded-xl p-4 transition-all group">
      <div className="flex items-start gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            {post.topic && <span className="bg-cyan-600/20 text-cyan-400 text-xs px-1.5 py-0.5 rounded">{post.topic}</span>}
          </div>
          <h3 className="text-text-primary font-semibold text-sm group-hover:text-cyan-300 transition-colors line-clamp-1">{post.title}</h3>
          <p className="text-text-muted text-xs mt-1 line-clamp-2">{textPreview}</p>
        </div>
      </div>
      <div className="flex items-center gap-3 mt-3 pt-3 border-t border-line text-xs text-text-muted">

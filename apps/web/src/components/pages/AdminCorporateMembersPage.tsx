...
export default function AdminCorporateMembersPage() {
  ...
  const fetchData = useCallback(() => {
    ...
    adminService.getCorporateMembers({
      ...
      startDate, endDate,
      status: status === '전체' ? undefined : STATUS_MAP[status],
      approvalStatus: approvalFilter !== '전체' ? APPROVAL_MAP[approvalFilter] : undefined,
      sortBy, sortOrder,
    })
      .then(res => {
        setData((res?.users ?? []) as CorporateMember[])
  ...
  const getStatusLabel = (m: CorporateMember) => m.isActive !== false ? '정상' : '정지'
    ...
    <AdminLayout>
      <div className="space-y-5">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-text-primary text-xl font-bold">기업회원 관리</h2>
          <span className="text-text-secondary text-sm ml-auto">{loading ? '로딩 중...' : `총 ${total.toLocaleString()}개사`}</span>
        </div>
            ...
            <div className="flex gap-1">
              {APPROVAL_OPTIONS.map(s => (
                <button key={s} onClick={() => { setApprovalFilter(s); setPage(1) }}
                  className={`px-3 py-1.5 rounded-lg text-base border transition-colors ${approvalFilter === s ? 'bg-blue-600/20 text-blue-300 border-blue-500/30' : 'bg-bg-tertiary text-text-secondary border-line hover:bg-line-light'}`}>
                  {s === '전체' ? '승인전체' : s}
                </button>
              ))}
            </div>
        ...
        {selected.size > 0 && (
          <div className="flex items-center gap-3 bg-bg-tertiary/50 border border-line rounded-xl px-4 py-3">
            <span className="text-text-secondary text-sm font-medium">{selected.size}개사 선택됨</span>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-line">
                  {data.length === 0 ? (
                    ...
                    const approvalColor = approval === 'approved' ? 'bg-accent-light text-accent' : approval === 'rejected' ? 'bg-accent-light text-accent-text' : 'bg-amber-500/20 text-amber-400'
                    return (
                          ...
                          {approval === 'pending' && (
                            <div className="flex gap-1">
                              <button onClick={() => setApprovalModal({ open: true, userId: m._id, action: 'approved', reason: '' })}
                                className="text-[10px] px-2 py-0.5 bg-accent hover:bg-accent-hover text-text-primary rounded transition-colors">
                                승인
                              </button>
                              <button onClick={() => setApprovalModal({ open: true, userId: m._id, action: 'rejected', reason: '' })}
                                className="text-[10px] px-2 py-0.5 bg-red-600 hover:bg-red-700 text-text-primary rounded transition-colors">
                                거절
                              </button>
                            </div>
                          )}
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-bg-secondary border border-line rounded-2xl w-full max-w-md p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-text-primary font-bold">{modal.type === 'notify' ? '알림 발송' : '포인트 지급'}</h3>
              <button onClick={() => setModal({ open: false, type: null })} className="text-text-secondary hover:text-text-primary">
                <X className="w-5 h-5" />
              </button>
            </div>
            <p className="text-text-secondary text-sm">{selected.size}개사에게 일괄 처리합니다</p>
            {modal.type === 'notify' ? (
              <>

            ...
            <div className="flex gap-1">
              {APPROVAL_OPTIONS.map(s => (
                <button key={s} onClick={() => { setApprovalFilter(s); setPage(1) }}
                  className={`px-3 py-1.5 rounded-lg text-xs border transition-colors ${approvalFilter === s ? 'bg-blue-600/20 text-blue-300 border-blue-500/30' : 'bg-bg-tertiary text-text-secondary border-line hover:bg-line-light'}`}>
                  {s === '전체' ? '승인전체' : s}
                </button>
              ))}
            </div>

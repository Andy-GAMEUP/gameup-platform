  ...
  return (
    <div className="min-h-screen bg-bg-primary">
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        <Link href={`/partner/${partnerId}`} className="flex items-center gap-1.5 text-text-secondary hover:text-text-primary text-sm mb-5 transition-colors">
          <ArrowLeft className="w-4 h-4" /> 채널로 돌아가기
        </Link>

        <article className="bg-bg-secondary border border-line rounded-xl p-6">
          <div className="flex items-center gap-2 mb-3 flex-wrap">
            {post.topicGroup && <span className="bg-bg-tertiary text-text-secondary text-xs px-2 py-0.5 rounded">{post.topicGroup}</span>}
          ...
          <div className="text-text-secondary text-sm leading-relaxed break-words mb-5
          ...
          {post.images?.length > 0 && (
            <div className="grid grid-cols-2 gap-2 mb-5">
              {post.images.map((img, i) => (
                <Image key={i} src={img} alt={`이미지 ${i + 1}`} width={800} height={256} className="w-full rounded-lg border border-line object-cover max-h-64" unoptimized />
              ))}
            </div>
          )}

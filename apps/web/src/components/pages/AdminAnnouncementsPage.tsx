...
const PRIORITY_LABELS: Record<string, { label: string; color: string }> = {
  ...
  urgent: { label: '긴급', color: 'text-accent-text' },
}
    ...
    <AdminLayout>
                ...
                return (
                  <tr key={a._id} className="border-b border-line/50 hover:bg-bg-tertiary/30">
                    ...
                    <td className="px-4 py-3">
                      {a.isPublished ? (
                        <span className="text-accent text-xs">게시됨</span>
                      ) : (
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => setShowForm(false)}>
          <div className="bg-bg-secondary border border-line rounded-xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-text-primary font-bold text-lg mb-4">{editTarget ? '공지사항 편집' : '공지사항 작성'}</h3>
            <div className="space-y-4">
              <div>
                <label className="block text-text-secondary text-xs mb-1">제목 *</label>
                <input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} className="w-full bg-bg-tertiary border border-line text-text-primary rounded px-3 py-2 text-sm" placeholder="공지사항 제목" />
              </div>
              <div>
                <label className="block text-text-secondary text-xs mb-1">내용 *</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-text-secondary text-xs mb-1">대상</label>
                  <select value={form.targetRole} onChange={(e) => setForm({ ...form, targetRole: e.target.value as typeof form.targetRole })} className="w-full bg-bg-tertiary border border-line text-text-primary rounded px-3 py-2 text-sm">
                ...
                <div>
                  <label className="block text-text-secondary text-xs mb-1">만료일 (선택)</label>
                  <input type="date" value={form.expiresAt} onChange={(e) => setForm({ ...form, expiresAt: e.target.value })} className="w-full bg-bg-tertiary border border-line text-text-primary rounded px-3 py-2 text-sm" />
                </div>
              </div>

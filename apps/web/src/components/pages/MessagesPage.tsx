    ...
    <div className="flex h-[calc(100vh-64px)] bg-[#0a0a0f]">
      <div className="w-80 flex-shrink-0 border-r border-slate-800 flex flex-col">
        <div className="px-4 py-4 border-b border-slate-800">
          <h2 className="text-white font-bold text-lg">메시지</h2>
        </div>
            ...
            rooms.map((room) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3 hover:bg-slate-800/50 transition-colors text-left border-b border-slate-800/50 ${isActive ? 'bg-red-500/10 border-l-2 border-l-red-500/30' : ''}`}
                >
                  <div className="w-10 h-10 rounded-full bg-gradient-to-br from-red-500 to-orange-600 flex items-center justify-center text-white text-sm font-bold flex-shrink-0">
                    {other?.username?.[0]?.toUpperCase() ?? '?'}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      <span className="text-white text-sm font-medium truncate">{other?.username ?? '알 수 없음'}</span>
                      {room.lastMessageAt && (
                        <span className="text-slate-500 text-xs ml-1 flex-shrink-0">{relativeTime(room.lastMessageAt)}</span>
                      )}
                    </div>
      ...
      {activeRoom ? (
        <div className="flex-1 flex flex-col min-w-0">
          <div className="px-4 py-3 border-b border-slate-800 flex items-center gap-3 bg-slate-900/50">
            ...
            <span className="text-white font-medium">{getOtherParticipant(activeRoom)?.username ?? '알 수 없음'}</span>
          </div>
              ...
              return (
                <div key={msg._id} className={`flex items-end gap-2 ${isMine ? 'flex-row-reverse' : 'flex-row'}`}>
                  ...
                  <div className={`max-w-[60%] flex flex-col ${isMine ? 'items-end' : 'items-start'}`}>
                    <div className={`px-4 py-2 text-sm ${isMine ? 'bg-red-600 text-white rounded-2xl rounded-br-sm' : 'bg-slate-800 text-white rounded-2xl rounded-bl-sm'}`}>
                      {msg.content}
                    </div>
                    <span className="text-slate-500 text-xs mt-1">{relativeTime(msg.createdAt)}</span>
                  </div>
                </div>
              )
            })}
          ...
          <div className="px-4 py-3 border-t border-slate-800 bg-slate-900/50 flex gap-2">
            <input
              type="text"
              value={input}
              ...
              className="flex-1 bg-slate-800 border border-slate-700 rounded-xl px-4 py-2 text-white text-sm placeholder-slate-500 outline-none focus:border-red-500/50 transition-colors"
            />
            <button
              onClick={handleSend}
              disabled={!input.trim()}

...
export default function PaymentsPage() {
  ...
  const [period,           setPeriod]           = useState<Period>(() => (typeof window !== 'undefined' ? (localStorage.getItem('payments_period') as Period) : null) ?? '30d')
  const [customFrom,       setCustomFrom]       = useState(() => (typeof window !== 'undefined' ? localStorage.getItem('payments_customFrom') : null) ?? toYMD(new Date(Date.now() - 29 * 86400000)))
  const [customTo,         setCustomTo]         = useState(() => (typeof window !== 'undefined' ? localStorage.getItem('payments_customTo') : null) ?? toYMD(new Date()))
  ...
  useEffect(() => {
    ...
    localStorage.setItem('payments_customFrom', customFrom)
    localStorage.setItem('payments_customTo', customTo)
  ...
  const downloadCSV = () => {
    ...
    a.href = url
    const { startDate, endDate } = buildDates(period)
    a.download = `payments_${startDate}_${endDate}.csv`
    a.click()
  ...
  return (
    <div className="space-y-4 min-w-0">
      ...
      {/* 테이블 */}
      {!queried ? (
        <div className="text-center py-16 text-text-muted text-sm">조회 버튼을 눌러 결제 내역을 확인하세요</div>
      ) : loading ? (

...
export default function GameSuggestModal({ isOpen, onClose, publishingType }: Props) {
  ...
  return (
    <div className="fixed inset-0 bg-bg-overlay z-50 flex items-center justify-center p-4" onClick={e => { if (e.target === e.currentTarget) handleClose() }}>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {ADDITIONAL_SERVICES.map(service => (
                    <label key={service} className="flex items-center gap-2.5 cursor-pointer p-2.5 rounded-xl bg-bg-tertiary border border-line hover:border-line transition-colors">
                      <input
                        type="checkbox"

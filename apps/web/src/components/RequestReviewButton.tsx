...
export default function RequestReviewButton({
  ...
  const showChecklist = !isUnderReview && !!checklist?.length
  const showDisabledReason = !isUnderReview && extraDisabled && !checklist?.length && !!extraDisabledTitle
  const tooltipLines: string[] = showDisabledReason && extraDisabledTitle
      ...
      <div className="relative group inline-flex">
        ...
        {showChecklist ? (
          <div className="absolute top-full right-0 mt-2 hidden group-hover:block z-50 pointer-events-none">
            <div className="bg-bg-primary border border-line rounded-lg shadow-lg px-3 py-2 text-xs whitespace-nowrap">
              <p className="leading-5 font-semibold text-text-primary mb-0.5">심사 등록 조건</p>
              {checklist!.map((item, i) => (
                <p key={i} className={`leading-5 flex items-center gap-1.5 ${item.done ? 'text-emerald-400' : 'text-text-secondary'}`}>
                  {item.done ? <Check className="w-3 h-3 flex-shrink-0" /> : <span className="w-3 h-3 flex-shrink-0 rounded-full border border-text-muted" />}
                  {item.label}
                </p>
              ))}
            </div>
